...
#warning - Richard: This is an imperfect solution. Commenting out the following line prevents the sidebar from completely filling the screen before pushing the new center view. To prevent the entire block from firing, disable Bounce Animations, but if we just disable Bounce Animations, you can see the status bar for a brief moment while the new view is being pushed.
